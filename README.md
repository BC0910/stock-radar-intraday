...
GitHub Actions 一天跑兩次：台灣時間約 11:05、13:05（週一到週五），對應 `.github/workflows/intraday.yml`
裡的 UTC cron `5 3 * * 1-5` 與 `5 5 * * 1-5`。刻意沒排在整點，因為 GitHub Actions 的排程觸發在每個
整點是全站最壅塞的時刻，容易延遲或被跳過，錯開幾分鐘可以降低這個機率。非交易日（週末/假日/尚未開
盤）會偵測即時報價時間戳不是今天，自動跳過、不推播不寫檔。
